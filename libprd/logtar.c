...
static TREE *log_tar_file (NBLK *n, struct stat *stbuf, int entry_count)
{
...
static int read_tar_node (NBLK *n)
{
	int c;
	int zero_cnt;
	int entry_count;
	...
	while (TRUE)
	{
		...
		if (!tar_is_valid_hdr())
			break;
		tar_get_stbuf(&stbuf);
		t = log_tar_file(n, &stbuf, ++entry_count);
...
int log_tar (void)
{
	...
	xaddstr(gbl(win_commands), msgs(m_logtar_ltf));
	waddstr(gbl(win_commands), msgs(m_logtar_frodev));
