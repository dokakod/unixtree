...
static char *tar_make_link_name (char *link_name)
{
	char *p;
...
static int tar_hard_link (const char *path)
{
	char ln[MAX_PATHLEN];
	int c;

	tar_make_link_name(ln);
	if (os_link(ln, path))
	{
		c = errsys(ER_CLF);
		return (c);
...
static int tar_soft_link (const char *path)
{
	int c;
#if X_IFLNK
	char ln[MAX_PATHLEN];
...
static int extract_tar_file (FBLK *f)
{
	NBLK *n;
	ABLK *a;
	...
	rc = tar_extract_file(new_name);

	return (rc);
}

static int ext_tar_file (void *data)
{
	int i;

	i = extract_tar_file(gbl(scr_cur)->cur_file);
	if (i < 0)
		return (i);
	return (1);
}

void do_tar_extract (void)
{
	werase(gbl(win_commands));
	wmove(gbl(win_commands), 0, 0);
	...
	extract_tar_file(gbl(scr_cur)->cur_file);
	tar_close(TRUE);
	FREE(ai->a_buffer);
}

void do_tag_tar_extract (void)
{
	BLIST *taglist;
	BLIST *first_file;
...
int tar_extract_file (const char *path)
{
	int num_recs;
	int out_fd;
	int i;
...
int tar_extract_temp (FBLK *f, const char *name)
{
	NBLK *n;
	ABLK *a;
