...
static TREE *log_bar_file (NBLK *n, struct stat *stbuf, int entry_count)
{
...
static int read_bar_node (NBLK *n)
{
	int c;
	int zero_cnt;
	int entry_count;
	...
	while (TRUE)
	{
		...
		if (!bar_is_valid_hdr())
			break;
		bar_get_stbuf(&stbuf);
		t = log_bar_file(n, &stbuf, ++entry_count);
...
int log_bar (void)
{
	...
	xaddstr(gbl(win_commands), msgs(m_logbar_lbf));
	waddstr(gbl(win_commands), msgs(m_logbar_frodev));
