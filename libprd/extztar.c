...
static char *ztar_make_link_name (char *link_name)
{
	char *p;
...
static int ztar_hard_link (const char *path)
{
	char ln[MAX_PATHLEN];
	int c;

	ztar_make_link_name(ln);
	if (os_link(ln, path))
	{
		c = errsys(ER_CLF);
		return (c);
...
static int ztar_soft_link (const char *path)
{
	int c;
#if X_IFLNK
	char ln[MAX_PATHLEN];
...
static int extract_ztar_file (FBLK *f)
{
	NBLK *n;
	ABLK *a;
	...
	if (tar_position(a->arch_name, f->archive_vol, f->archive_loc))
		return (1);

	/* read the file */

	rc = ztar_extract_file(new_name);

	return (rc);
}

static int ext_ztar_file (void *data)
{
	int i;

	i = extract_ztar_file(gbl(scr_cur)->cur_file);
	if (i < 0)
		return (i);
	return (1);
}

void do_ztar_extract (void)
{
	werase(gbl(win_commands));
	wmove(gbl(win_commands), 0, 0);
	...
	extract_ztar_file(gbl(scr_cur)->cur_file);
	tar_close(TRUE);
	FREE(ai->a_buffer);
}

void do_tag_ztar_extract (void)
{
	BLIST *taglist;
	BLIST *first_file;
...
int ztar_extract_file (const char *path)
{
	int num_recs;
	int out_fd;
	int i;
	...
	for (v=0; v<vol_cnt; v++)
	{
		...
		if (v)
		{
			while (TRUE)
			{
				if (tar_position(ai->a_name, old_vol+v, 0))
				{
					v = -1;
					break;
				}
...
int ztar_extract_temp (FBLK *f, const char *name)
{
	NBLK *n;
	ABLK *a;
