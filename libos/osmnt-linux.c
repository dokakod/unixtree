...
int os_mnt_read (void *md, MOUNT_INFO *mptr)
{
	 ...
	 * extract info
	 */
	strcpy(mptr->mt_dev, mnt->mnt_fsname);
	strcpy(mptr->mt_dir, mnt->mnt_dir);
	mptr->mt_flags = 0;
...
long os_mnt_space (const char *path)
{
	 ...
	 */
	free_amount = fsbuf.f_bavail * (fsbuf.f_bsize / 1024);
	if (free_amount < 0)
		free_amount = 0;
