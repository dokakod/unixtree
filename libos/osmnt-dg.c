...
int os_mnt_read (void *md, MOUNT_INFO *mptr)
{
	...
	while (TRUE)
	{
		...
		if (mnt_ent->mnt_type &&
			!(strcmp(mnt_ent->mnt_type, MNTTYPE_DGUX) == 0 ||
			  strcmp(mnt_ent->mnt_type, MNTTYPE_NFS)  == 0 ||
			  strcmp(mnt_ent->mnt_type, MNTTYPE_PC)   == 0) )
		{
			continue;
		}
...
long os_mnt_space (const char *path)
{
	 ...
	 */
	rc = statfs((char *)path, &fsbuf, sizeof(fsbuf), 0);
	if (rc)
		return (-1);
